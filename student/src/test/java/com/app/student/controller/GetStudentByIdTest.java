...
public class GetStudentByIdTest {
  ...
  void ShouldReturnOk_WhenIdExists() throws Exception {
    ...
    ResultActions response =
        mockMvc.perform(
            get("/getStudentById/{id}", universityNumber)
  ...
  void ShouldReturnNotFound_WhenIdDoesNotExist() throws Exception {
    given(studentService.getStudentById(ArgumentMatchers.any()))
        .willThrow(new StudentNotFoundException("Student Record Not Found!"));
    ResultActions response =
        mockMvc.perform(
            get("/getStudentById/{id}", universityNumber)
                .contentType(MediaType.APPLICATION_JSON)
