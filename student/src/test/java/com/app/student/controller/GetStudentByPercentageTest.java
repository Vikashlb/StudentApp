...
public class GetStudentByPercentageTest {
    ...
    void init() {
        ...
        students = Arrays.asList(
                new Student("22MX228", "Vikash", "Bojarajan",
                        "Computer Applications", 87.50, LocalDate.parse("2002-06-10")),
                new Student("22MX221", "Vikram", "Kumar",
                        "Computer Applications", 87.50, LocalDate.parse("2001-08-21"))
        );
    }
    ...
    void ShouldReturnOk_WhenDataIsAvailable() throws Exception {
        ...
        ResultActions response = mockMvc.perform(get("/getStudentByPercentage/{percentage}",percentage)
    ...
    void ShouldReturnNotFound_WhenDataIsNotAvailable() throws Exception {
        given(studentService.getStudentByPercentage(ArgumentMatchers.anyDouble()))
                .willThrow(new StudentNotFoundException("Student Record With Percentage : '"+ percentage +"' Not Found!"));
        ResultActions response = mockMvc.perform(get("/getStudentByPercentage/{percentage}",percentage)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(students)));
        ...
        response.andExpect(MockMvcResultMatchers.status().isNotFound())
                ...
                .andExpect(jsonPath("$.errorMessage").value("Student Record With Percentage : '"+ percentage +"' Not Found!"));
    }
}
