...
public class GetStudentByDepartmentTest {
    ...
    void ShouldReturnOk_WhenDataIsAvailable() throws Exception {
        ...
        ResultActions response = mockMvc.perform(get("/getStudentByDepartment/{department}",departmentName)
    ...
    void ShouldReturnNotFound_WhenDataIsNotAvailable() throws Exception {
        given(studentService.getStudentByDepartmentName(ArgumentMatchers.any()))
                .willThrow(new StudentNotFoundException("Student Record With Department : '"+ departmentName +"' Not Found!"));
        ResultActions response = mockMvc.perform(get("/getStudentByDepartment/{department}",departmentName)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(students)));
        ...
        response.andExpect(MockMvcResultMatchers.status().isNotFound())
                ...
                .andExpect(jsonPath("$.errorMessage").value("Student Record With Department : '"+ departmentName +"' Not Found!"));
    }
}
